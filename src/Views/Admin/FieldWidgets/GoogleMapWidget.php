 ...
 * @since   1.0.0
 */
class GoogleMapWidget extends BaseFieldWidget
{
    ...
    public function outputWidgetCore()
    {
        ...
        $wrapAttrs   = [
            ...
            'class' => 'axis3-field-widget axis3-google-map-widget',
        ];
        ...
        $this->render(
            'admin/field-widgets/google-map.php',
            [
                'addr_id'    => $this->getId(),
        ...
        wp_add_inline_script(
            'axis3-google-map',
            "jQuery(function (\$) { $('#{$containerId}').axis3GoogleMap(); });"
        );
    }

    public function onceAfterRender()
    {
        if (!wp_script_is('axis3-google-map')) {
            wp_enqueue_script(
                'axis3-google-map-api',
                add_query_arg('key', $this->args['apiKey'], 'https://maps.googleapis.com/maps/api/js'),
                [],
                false,
                true
            );
            wp_enqueue_script('axis3-google-map');
            wp_localize_script(
                'axis3-google-map',
                'axis3GoogleMap',
